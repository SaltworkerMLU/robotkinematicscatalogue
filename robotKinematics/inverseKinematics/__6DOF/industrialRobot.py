    ...
    def IK(self, TBW):
        ...
        for i in range(len(self.null_joint)):
            if self.null_joint[i] != 0:
                print("ATTENTION: Nullified joints have not been accounted for in Inverse Kinematics calculations.")
        ...
        for i in range(8):   
            # Theta1
            phi = np.arctan2(T06[1,3], T06[0,3])
            ...
            Joint[i, 0] = self.theta[0] + phi[i//4 % 2]
            ...
            Joint[i, 2] = self.inv_joint[2] * Joint[i, 2]
